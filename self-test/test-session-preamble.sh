...
EOF
# repo gamma: clean PR (no failure signal → should NOT surface)
...
if json="$(bash "$SEARCH" rescue --memory-dir "$MEM" --json 2>/dev/null)" \
  ...
  if [[ "$top_repo" == "foo" ]] && [[ "$top_matches" -ge 2 ]] && [[ "$n_results" -eq 2 ]] && [[ "$has_example" == "false" ]]; then
    pass "memory-search ranks foo first (matches=$top_matches), 2 results, example skipped"
  else
