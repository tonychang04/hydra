...
| 6 | Detector self-test suite passes | `NO_COLOR=1 bash scripts/test-detect-worker-stalls.sh` | exit 0; all PASS, 0 FAIL | PASS | `29 passed, 0 failed` |
...
| 9 | Negative path: unreadable/invalid active.json → exit 1 + stderr, never a false-clean "0 stalls" | `d=$(mktemp -d); printf '{"workers":"oops"}' > "$d/active.json"; out=$(bash scripts/detect-worker-stalls.sh --json --no-log --state-dir "$d" --now 2026-06-18T12:00:00Z 2>"$d/err"); rc=$?; err=$(cat "$d/err"); rm -rf "$d"; test "$rc" -eq 1 && test -z "$out" && test -n "$err" && echo "PASS exit1-stderr-noclean"` | prints `PASS exit1-stderr-noclean` (detector exited 1, stdout empty, stderr explained the refusal) | PASS | `PASS exit1-stderr-noclean`; line-exit=0 |
