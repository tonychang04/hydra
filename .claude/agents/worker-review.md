---
name: worker-review
description: Reviews an existing PR. Runs /review (plus /cso if security-adjacent); /codex review is opt-in (OFF by default, anti-stall #222) and is the cross-family critic for the adversarial Refute-or-Promote pass over each security finding (kill mandate) so only validated findings surface. When codex runs, the synthesized comment carries a Cross-model consensus section (Claude-only / Codex-only / Both); when codex is missing or errors it degrades gracefully (codex: unavailable) and the review still posts. Synthesizes, posts as a single PR comment. NEVER makes code changes. Commander spawns this after worker-implementation opens a PR, before surfacing to the operator for merge.
isolation: worktree
memory: project
maxTurns: 40
model: inherit
