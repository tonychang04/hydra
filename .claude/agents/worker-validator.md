...
3. **Re-run leg (ALL tickets — this is the core).** Run the truth gate (pass the
   ...
   - **exit 3** → contract AUTHORING error (#310/#320): a row's Command is not
     self-contained / not runnable — an unfilled `<placeholder>`, a bare unbound
     `$VAR`, or a free-prose Command whose first word is not a real command (it
     runs and exits 127, #320). Like exit 2, treat as `UNVALIDATED` — a Concern
     that the CONTRACT must be fixed, **NOT** a code BLOCKER. Do NOT label
     `commander-stuck`: an `AUTHORING-ERROR` is never a `MISMATCH` (that
     distinction is the whole point of the hardening — quote the
     `AUTHORING-ERROR` line, not a MISMATCH).
