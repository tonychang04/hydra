...
**Start with the dispatcher.** `using-hydra-skills/SKILL.md` is the entrypoint workers read FIRST in Step 0: before acting, enumerate this directory, match your task against each skill's `description`, and READ any skill with even a 1% chance of applying. It is the Hydra analogue of superpowers' `using-superpowers` dispatcher, adapted to the Step-0 context-read model (not the `Skill` tool). Worker agent definitions (`.claude/agents/worker-*.md`) point at it from their Step-0 sections.
...
---
name: <kebab-name>
version: <semver>          # REQUIRED. New hand-authored skills start at 1.0.0.
description: |
...
| `version` | yes | Semver. New hand-authored skills start at `1.0.0`. Bump on meaningful edits so workers can tell a current skill from a stale one. |
| `description` | yes | The trigger paragraph Claude's matcher keys on. |

### Provenance section (promoted skills only)

Skills generated by `scripts/promote-citations.sh` carry a **`## Promoted from memory`** section directly under the title, recording the source memory file, the load-bearing quote, the citation count at promotion, and the promotion date. It is fixed at promotion time — distinct from the machine-spliced `## Citations` block, which is the *live* citation tally and is rewritten idempotently on re-promotion. Hand-authored skills (like the seeds and the dispatcher) omit the provenance section.
...
2. Create `.claude/skills/<name>/SKILL.md` matching the shape above — including the required `version: 1.0.0` frontmatter field.
