...
#         count (citation count at promotion, for the provenance section).
#
# Skill-shape contract (#241): freshly-promoted skills carry `version: 1.0.0`
# frontmatter and a `## Promoted from memory` provenance section (source, quote,
# citation count, promotion date). Provenance is fixed at promotion time; the
# live tally stays in the spliced `## Citations` block.
#
...
build_skill_body() {
  ...
  local citations="$6"
  local count="${7:-3}"
  # Promotion date for the provenance section. Falls back to the global $today
  # (set in main), then to current UTC, so the function is self-contained even
  # when extracted and called in isolation (e.g. the test harness).
  local promoted="${today:-$(date -u +%Y-%m-%d)}"
...
- **Quote:** "@@QUOTE@@"
- **Citations at promotion:** @@COUNT@@ (across 3+ distinct tickets)
- **Promoted:** @@PROMOTED@@
- **Initial version:** 1.0.0 (machine-scaffolded; operator hardens before merge)

This skill was auto-generated by `scripts/promote-citations.sh` when the source
memory entry crossed the 3×3 citation threshold (`memory/memory-lifecycle.md`).
The provenance above is fixed at promotion time; the live citation tally lives
in the **Citations** section below.
...
TEMPLATE
  ...
  template="${template//@@PROMOTED@@/$promoted}"
...
process_entry() {
  ...
  if [[ -f "$full_skill_path" ]]; then
    ...
    build_skill_body "$combined_slug" "$description" "$quote" "$file" "$context" "$citations_block" "$count" \
      > "$full_skill_path"
    changed=1
  fi
