...
| `test-parse-citations.sh` | Operator pre-push, or CI | Regression test for `parse-citations.sh` (the learning-loop entry point). Thirteen cases: single marker → correct `<file>#"<quote>"` key + `count_delta:1`, duplicate-key aggregation, `TICKET:` ref attached + de-duped, `TICKET:`-absent empty list, zero markers → `{"citations":{}}`, quoted-quote verbatim key, stdin == `<report-file>` arg equivalence, `--ticket` flag overriding inline `TICKET:` and standalone, missing-cited-file warning (non-fatal), usage errors → exit 2, `-h`/`--help`, and whitespace trimming. No fixtures outside `mktemp`. Spec: `docs/specs/2026-06-08-test-parse-citations.md`. |
...
| `test-validate-citations.sh` | Operator pre-push, or CI | Regression test for `validate-citations.sh` (the citation-liveness check that gates skill promotion). Cases: flat vs `{citations:{}}` JSON shapes, quoted vs unquoted `#"..."` unwrap, live → exit 0, deleted quote → exit 1, case-sensitivity (case-mismatch is stale), missing referenced file, malformed key (no `#`), empty `{}` no-op, `_`-prefixed keys skipped, `--memory-dir`/`--citations` overrides honored, and usage errors → exit 2. Hermetic — `mktemp` fixtures only; never reads the real `memory/` tree or sets `HYDRA_MEMORY_BACKEND`. Spec: `docs/specs/2026-06-08-test-validate-citations.md`. |
