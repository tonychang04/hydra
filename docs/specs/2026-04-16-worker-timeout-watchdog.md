...
**Exit-5 (`flaky-tool-retry`) timeout-path routing.** On the *timeout* (not completion) path, exit 5 is the live-worker-blocked-on-a-flaky-tool case: Commander reads `tool` + `fallback` from the verdict JSON (or `<worktree>/.hydra/flaky-tool.json`), `SendMessage`s the live worker to fall back, labels `commander-flaky-tool-retry`, and does NOT `--rescue` / `commander-stuck`. A second `flaky-tool-retry` from the same worker (fallback also failed) or a subsequent genuine silence falls through to the normal rescue / stuck path — no infinite retry. Canonical routing entry + the CLAUDE.md one-liner: `docs/specs/2026-06-07-tick-stall-robustness.md` (ticket #242); detection contract: `docs/specs/2026-05-21-flaky-tool-probe-verdict.md` (ticket #190).
