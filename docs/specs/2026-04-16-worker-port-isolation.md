...
Constraint: Compose project names must be lowercase. Worker IDs that include uppercase letters (or characters outside `[a-z0-9_-]`) must be sanitized before being plugged into `COMPOSE_PROJECT_NAME` — typical pattern is `tr '[:upper:]' '[:lower:]' | tr -c '[:alnum:]-_' '-'`.
...
3. Writes a `docker-compose.override.yml` **in the worktree** (not in the target repo's committed tree) that remaps published ports to the allocated ones. Compose auto-merges `docker-compose.override.yml` with `docker-compose.yml`, so no flags are needed at `up` time. The override **must** use `ports: !override` (Compose 2.17+) rather than a plain list — Compose's default list-merge behavior is to *union* the two `ports` lists, which leaves the base's `5432:5432` binding active alongside the remapped one and reproduces the original host-port collision. `!override` replaces the list wholesale, which is what we want.
...
- **Risk: `/dev/tcp` not available.** The allocator uses bash's `/dev/tcp` builtin for port probing. It's a bash builtin present since bash 2.04 and works on every mainstream distro + macOS under the `#!/usr/bin/env bash` shebang. It does NOT work under zsh or dash, which is why the fixture driver + allocator explicitly specify bash.
- **Risk: Compose merges override `ports` instead of replacing.** A naive `ports: [host:container]` in the override file *adds* to the base compose's ports list, so the base's `5432:5432` binding is still active — host-port collision unchanged. Fix: always use `ports: !override` (Compose 2.17+, shipped late 2023). Workers on Compose < 2.17 must instead pass `-f` explicitly with a custom file that rewrites ports, or upgrade Compose. The fixture + worker-implementation docs enforce the `!override` idiom; the smoke test would fail loudly without it.
