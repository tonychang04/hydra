...
**PR Shepherd** (read-only): `scripts/pr-shepherd.sh [--repo R] [--json]` classifies Commander-authored open PRs and flags orphans (open PR, no `active.json` worker) — run at session start (re-attach orphans) and on the autopickup tick (route in-flight PRs). Spec: `docs/specs/2026-05-23-pr-shepherd.md`.

## Worker timeout watchdog

On every tick touching `state/active.json` (and on every worker COMPLETION, #219), probe workers >12 min with no completion log via `scripts/rescue-worker.sh --probe`: a rescue verdict → `--rescue` (commit + rebase + push) + label `commander-rescued` + respawn with "resume from rescue"; `nothing-to-rescue`/`push-only` → `commander-stuck`; rebase-conflict (exit 3) → surface. Review variant `--probe-review <pr>`; exit 4 → `/review` inline. Workers also prevent the 600s stall up front (anti-stall discipline). Specs: `docs/specs/2026-04-16-worker-timeout-watchdog.md`, `docs/specs/2026-04-17-review-worker-rescue.md`, `docs/specs/2026-04-17-rescue-worker-index-lock.md`, `docs/specs/2026-05-23-auto-rescue-on-completion.md`, `docs/specs/2026-05-24-worker-anti-stall-discipline.md`.
...
`autopickup every N min` runs a cron-like `/loop` (no daemon; state `state/autopickup.json`). Default-on (`auto_enable_on_session_start`); opt out with `./hydra --no-autopickup` or `HYDRA_NO_AUTOPICKUP=1`. Each tick chains preflight → Monday retro → daily digest → daily skill-promotion → pick → spawn to capacity (full step list in the scheduled-autopickup + skill-promotion specs). Quiet except on state changes; two consecutive rate-limit hits auto-disable. `autopickup off` exits; in-flight workers finish.

Specs: `docs/specs/2026-04-16-scheduled-autopickup.md`, `docs/specs/2026-04-16-autopickup-default-on.md`, `docs/specs/2026-04-17-scheduled-retro.md`, `docs/specs/2026-04-17-daily-digest.md`, `docs/specs/2026-04-17-skill-promotion-automation.md`.

## Memory hygiene (runs silently)

Per-ticket: parse `MEMORY_CITED:` → bump `state/memory-citations.json` (flag `count>=3` across 3+ tickets). Every 10 tickets (or `compact memory`): merge dups, archive stale → `memory/archive/`, draft `commander-skill-promotion` PRs. Pre-spawn: prepend a Memory Brief. Skill promotion runs once/day via the autopickup tick (`scripts/promote-citations.sh` → `.claude/skills/`). Canonical tooling (don't reinvent): `scripts/{parse-citations,validate-learning-entry,validate-citations}.sh`. Rules: `memory/memory-lifecycle.md`.
...
On `retro` (or Monday ≥ 09:00 local via the autopickup tick): read 7 days of `logs/*.json` + citation deltas + recent `escalation-faq.md`; write `memory/retros/YYYY-WW.md` (Shipped / Stuck / Escalations / Citation leaderboard / Proposed edits — edits need ≥5 data points). Stamp `state/autopickup.json:last_retro_run` (idempotent; overwrite on re-run); chat summary + pointer. Then `scripts/retro-file-proposed-edits.sh <week>` auto-files each Proposed-edits bullet as a `commander-ready` issue. Specs: `docs/specs/2026-04-16-retro-workflow.md`, `docs/specs/2026-04-17-scheduled-retro.md`, `docs/specs/2026-04-17-retro-auto-file.md`.
...
If auto-enabled, add: `Autopickup entered automatically (opt-out). Disable with `autopickup off` or relaunch with `./hydra --no-autopickup`.` Then wait — `/loop` handles pickups; first tick after `interval_min` so the operator can countermand. Spec: `docs/specs/2026-04-16-autopickup-default-on.md`.

**Append three health one-liners** (silent when healthy; append the script's stdout verbatim when not):
- **Connectors** — `scripts/hydra-connect.sh --status --quiet`. Spec: `docs/specs/2026-04-17-connector-wizard.md`.
- **CLAUDE.md size** — `scripts/check-claude-md-size.sh --quiet` (at WARN, trim a section to a spec before adding routing). Spec: `docs/specs/2026-05-20-claudemd-size-graduated-bands.md`.
- **Skill promotion** — `scripts/promote-citations.sh --greeting-count`. Spec: `docs/specs/2026-04-17-skill-promotion-automation.md`.
