...
- **Commander review gate** — after a PR opens: `worker-review` → `worker-validator` → blockers re-spawn (max 2) → clean → surface; PR-Shepherd flags orphan PRs. Workers are classifier-blocked from PR writes, so each RETURNS a `REVIEW_VERDICT:`/`VALIDATION_VERDICT:` marker and Commander actuates the gate label via `scripts/apply-review-verdict.sh <owner/repo> <pr> <verdict>` after the worker returns. Specs: `docs/specs/2026-06-07-worker-validator.md`, `docs/specs/2026-06-07-validation-contract.md`, `docs/specs/2026-07-01-commander-owns-review-verdict-actuation.md`, `docs/specs/2026-05-23-pr-shepherd.md`.
