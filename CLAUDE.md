...
Workers can hit ~18-min stream-idle timeouts with partial work unpushed. On every tick touching `state/active.json` AND on every worker COMPLETION (#219), run `scripts/rescue-worker.sh --probe`: rescuable → `--rescue` (commit+rebase+push) + label `commander-rescued` + respawn ("resume from rescue"); non-rescuable → `commander-stuck`; rebase-conflict (exit 3) → surface; review variant `--probe-review <pr>` exit 4 → `/review` inline; `flaky-tool-retry` (exit 5) → worker is ALIVE — `SendMessage` it the marker's `fallback`, label `commander-flaky-tool-retry`; do NOT `--rescue` / `commander-stuck`. Verdict→action table + anti-stall rules in specs: `docs/specs/2026-04-16-worker-timeout-watchdog.md`, `2026-04-17-review-worker-rescue.md`, `2026-04-17-rescue-worker-index-lock.md`, `2026-05-23-auto-rescue-on-completion.md`, `2026-05-24-worker-anti-stall-discipline.md`, `2026-06-07-tick-stall-robustness.md`.
...
**Append three health one-liners** (silent when healthy; append stdout verbatim when not): connectors `scripts/hydra-connect.sh --status --quiet`; CLAUDE.md size `scripts/check-claude-md-size.sh --quiet` (at WARN trim a section to a spec before adding routing); skill promotion `scripts/promote-citations.sh --greeting-count` (>0 ⇒ N PR(s) awaiting review). Specs: `docs/specs/2026-04-17-connector-wizard.md`, `2026-05-20-claudemd-size-graduated-bands.md`, `2026-04-17-skill-promotion-automation.md`.
